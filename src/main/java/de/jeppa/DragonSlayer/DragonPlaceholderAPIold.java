        ...
        if (this.canRegister()) {
            try {
                ...
                if (PAPIHook != null) {
                    ...
                    try {
                        PAPIHook.invoke(PlaceholderAPI.class, this.getIdentifier(), this);
    ...
    public String onPlaceholderRequest(Player player, String identifier) {
        if (identifier.equals("prefix")) {
            ...
            if (identifier.contains("time") || identifier.equals("nextmap") || identifier.contains("reset")) {
                ...
                if (identifier.replaceAll("nexttimehm([s]?)", "nexttime").startsWith("nexttime_")) {
    ...
    private String getTimerParsed(String Mapname, String identifier) {
        ...
        if (nextSpawn != null) {
            ...
            if (identifier.equals("nexttimehms")) {
                return String.format("%s:%s:%s", String.valueOf(Integer.parseInt(nextSpawn[0]) * 24 + Integer.parseInt(nextSpawn[1])),
                        nextSpawn[2], nextSpawn[3]);
            }

            if (identifier.equals("nexttimehm")) {
                return String.format("%s:%s", String.valueOf(Integer.parseInt(nextSpawn[0]) * 24 + Integer.parseInt(nextSpawn[1])),
                        nextSpawn[2]);
            }
        }
        ...
        if (nextReset != null) {
            ...
            if (identifier.equals("nextresethms")) {
                return String.format("%s:%s:%s", String.valueOf(Integer.parseInt(nextReset[0]) * 24 + Integer.parseInt(nextReset[1])),
                        nextReset[2], nextReset[3]);
            }

            if (identifier.equals("nextresethm")) {
                return String.format("%s:%s", String.valueOf(Integer.parseInt(nextReset[0]) * 24 + Integer.parseInt(nextReset[1])),
                        nextReset[2]);
            }
        }
